...
class MKSOutputDevice(NetworkedPrinterOutputDevice):
    ...
    def init_translations(self):
        self._translations = {
            ...
            "file_send_failed": catalog.i18nc("@info:status", "Send file to printer failed. Check printer SD card."),
    ...
    def check_valid_filename(self, filename):
        ...
        if file_already_exists or filename_too_long:
            if self.is_auto_file_renaming_enabled():
                filename = self.check_valid_filename(utils.generate_new_filename(self.sdFiles, filename, max_filename_len))
            elif file_already_exists:
                filename = self.check_valid_filename(self.show_exists_dialog(filename))
            else: #filename_too_long
        ...
        if utils.contains_cyrillic(filename):
            filename = self.check_valid_filename(utils.transliterate(filename))
    ...
    def requestWrite(self,
        ...
        # Serhat patch: OutputDeviceManager.writeStarted may not fire on the
        # WiFi upload path, so call add_preview directly to guarantee that
        # ;simage and ;;gimage headers are injected before the gcode is sent.
        try:
