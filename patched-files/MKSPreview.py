...
def take_screenshot():
    ...
    # Serhat patch: Snapshot.snapshot() in Cura 5.13 often returns None due to
    # OpenGL timing issues ("Failed to crop the snapshot after 10 attempts").
    # Fallback chain: snapshot -> isometricSnapshot -> backend.getLatestSnapshot.
    img = None
    try:
    ...
    if img is None:
        try:
            if hasattr(Snapshot, "isometricSnapshot"):
                ...
                if img is not None:
                    Logger.log("d", "MKS SERHAT: using Snapshot.isometricSnapshot fallback")
        except Exception as e:
    ...
    if img is None:
        try:
            ...
            if backend is not None and hasattr(backend, "getLatestSnapshot"):
                ...
                if img is not None:
                    Logger.log("d", "MKS SERHAT: using CuraEngineBackend.getLatestSnapshot fallback")
        except Exception as e:
            Logger.log("w", "MKS: backend snapshot fallback failed: " + str(e))
    if img is None:
        Logger.log("w", "MKS SERHAT: no snapshot available (snapshot, isometricSnapshot, backend all failed)")
    return img

def generate_preview(global_container_stack, image):
    screenshot_string = ""
    # Serhat patch: default fallback sizes so preview is always emitted
    simage = 100
    gimage = 200
...
def add_preview(self):
    ...
    # Serhat debug: dump state before modification loop
    Logger.log("d", "SERHAT: gcode_dict has %d plates, keys=%s, screenshot_string len=%d" % (len(gcode_dict), list(gcode_dict.keys()), len(screenshot_string)))

    for plate_id in gcode_dict:
        gcode_list = gcode_dict[plate_id]
        Logger.log("d", "SERHAT: processing plate %s, gcode_list has %d items, item[0] len=%d" % (str(plate_id), len(gcode_list), len(gcode_list[0]) if gcode_list else 0))
        if len(gcode_list) < 2:
        ...
        dict_changed = True
        Logger.log("d", "SERHAT: plate %s modified, new item[0] len=%d, starts with: %s" % (str(plate_id), len(gcode_list[0]), gcode_list[0][:60].replace("\n","\\n").replace("\r","\\r")))

    Logger.log("d", "SERHAT: dict_changed=%s" % str(dict_changed))
    if dict_changed:
        setattr(scene, "gcode_dict", gcode_dict)
        Logger.log("d", "SERHAT: gcode_dict re-set on scene")
